...
//Is used as a wrapper to allow the boardroom to stake and withdraw a "share" token aswell as keep balances of all those who have deposited.
...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    //Implicit use of msg sender to check the msg sender has a balance.
    ...
    modifier updateReward(address member) {
        if (member != address(0)) {
            ...
            //Updates the reward earned amount.
            seat.rewardEarned = earned(member);

            //Updates the snapshot index to be the current one.
    ...
    //Modifier to update the current holdings for the owner.
    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        require(amount > 0, "Boardroom: Cannot stake 0");
        super.stake(amount); //Only one inheritance so its fine to call - goes from the share wrapper.
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            members[msg.sender].epochTimerStart = treasury.epoch(); //Reset timer.
            members[msg.sender].rewardEarned = 0;//Reset reward earnt.
            main.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        main.safeTransferFrom(msg.sender, address(this), amount);
